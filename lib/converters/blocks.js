  ...
  for (const file of files) {
    ...
    tasks.push({
      ...
      run: () => {
        ...
        registryLines.push(
          `\t\tRegistry.register(Registries.BLOCK, Identifier.of(${JSON.stringify(modId)}, ${JSON.stringify(name)}),\n` +
            `\t\t\tnew Block(AbstractBlock.Settings.create().strength(2.0f, 6.0f))); // TODO: review hardness/resistance/material from ${file}`
        );
        ...
        if (componentKeys.length) {
          ...
          logger.ok('blocks', `Block "${identifier}" converted (model, blockstate, item model, loot table, lang)`);
        }
      }
    });
  }
