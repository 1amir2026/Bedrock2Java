...
function scanAddon(addonPath) {
  ...
  // Exclude standard internal RP/BP subfolder names from being treated as pack roots themselves -
  // e.g. "textures/items" is a texture category folder, not a Behavior Pack's "items" folder,
  // but naively checking for an "items" subfolder would otherwise misidentify it as a BP.
  ...
  for (const child of children) {
    ...
    if (full === result.rp || full === result.bp) continue; // already assigned via Case 1
    if (INTERNAL_SUBFOLDER_NAMES.has(child.toLowerCase())) continue; // not a pack root candidate
