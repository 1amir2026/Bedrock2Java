...
async function runConversion(opts) {
  const { addonPath, outDir, modId, modName, modVersion, authorName, description, selectedCategories } = opts;

  ui.heading('1. Scanning Bedrock Add-On');
  const scan = scanAddon(addonPath);
  if (!scan.rp && !scan.bp) {
    ui.err(`Could not find a Resource Pack or Behavior Pack inside: ${addonPath}`);
    process.exit(1);
  }
